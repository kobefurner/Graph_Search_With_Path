...
class MC{
    public:
      MC();
      MC(const GameState&);
      void buildGraph(const GameState&);
      void solve();
      void getGraphPath(deque<GameState>& solution, const GameState& src, const GameState& target);
    private:
...
void MC::getGraphPath(deque<GameState>& solution, const GameState& src, const GameState& target){
  graph.getPath(solution, src, target);
}

MC::MC(const GameState& start){
   graph.addVertex(start);
   buildGraph(start);
}


void MC::buildGraph(const GameState& curr){
      ...
      for(int m = 0; m <= 2; m++){ //Number of missionaries in the boat
        for(int c = 0; c <= 2; c++){ //Number of cannibals in the boat
          ...
          if(curr.isBoatLeft()){
            ...
            if(state.isValidState() && !graph.vertexExists(state)){
